...
export async function initDb() {
    ...
    await db.exec('CREATE TABLE IF NOT EXISTS Triage (id INTEGER PRIMARY KEY, nurse_id INTEGER NOT NULL, systolicPreassure INTEGER NOT NULL, diastolicPreassure INTEGER NOT NULL, heartRate INTEGER NOT NULL, respiratoryRate INTEGER NOT NULL, bodyTemperature INTEGER NOT NULL, oxygenSaturation INTEGER NOT NULL, painLevel INTEGER, symptoms TEXT, triageCategory TEXT, FOREIGN KEY (id) REFERENCES Attend(id), FOREIGN KEY (nurse_id) REFERENCES Nurse(id), FOREIGN KEY (triageCategory) REFERENCES TriageCategory(id))');
