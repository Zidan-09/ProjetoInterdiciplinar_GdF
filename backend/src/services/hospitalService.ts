...
export class HospitalServices {
    static async register(data: Patient) {
        ...
        // );
        const patient = (await db).exec('INSERT INTO Patients (name, dob, maritalStatus, cpf, rg, contacts, gender, healthPlan, address) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)', [data.name, data.dob, data.maritalStatus, data.cpf, data.rg, data.contact, data.gender, data.healthPlan, data.address])
