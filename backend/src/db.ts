...
export async function initDb() {
    ...
    await db.exec('CREATE TABLE IF NOT EXISTS Triage (triage_id INTEGER PRIMARY KEY, nurse_id INTEGER NOT NULL, systolicPreassure INTEGER NOT NULL, diastolicPreassure INTEGER NOT NULL, heartRate INTEGER NOT NULL, respiratoryRate INTEGER NOT NULL, bodyTemperature INTEGER NOT NULL, oxygenSaturation INTEGER NOT NULL, painLevel INTEGER, symptoms TEXT, triageCategory TEXT, FOREIGN KEY (triage_id) REFERENCES CareFlow(id), FOREIGN KEY (nurse_id) REFERENCES Nurse(id), FOREIGN KEY (triageCategory) REFERENCES TriageCategory(id))');
